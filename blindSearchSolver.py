...
class NonogramBlindSearchSolver:
        ...
        with open(testcase, 'r') as f:
            for line in f:
                ...
                if count_line == 0:
                    ...
                    row = [int(i) for i in line_items]
                    self.row_constraints.append(row)
                else:
                    col = [int(i) for i in line_items]
                    self.col_constraints.append(col)
                count_line += 1
    ...
    def generatePossibleRowForms(self):
        for row_constraint in self.row_constraints:
            ...
            num_groups = len(row_constraint)
            num_empty = self.width - sum(row_constraint) - num_groups + 1
            options = self.generateOptions(num_groups, num_empty)
            for opt in options:
                res_opt = []
                for i in range(len(opt)):
                    if i == 0 and opt[i] != 0:
    ...
    def generateOptions(self, num_groups, num_empty):
        if num_groups == 0:
            return [[0] * num_empty]
        options = []
        for i in range(num_empty + 1):
            sub_options = self.generateOptions(num_groups - 1, num_empty - i)
            for sub_opt in sub_options:
                options.append([i] + sub_opt)
        return options
    ...
    def solveDFS(self, state=None):
        ...
        for i in range(1, len(self.possibleRowForms[index]) + 1):
            child_state = deepcopy(state)
            child_state[index] += i
            if self.isGoal(child_state):
                return True
            if self.solveDFS(child_state):
                return True
